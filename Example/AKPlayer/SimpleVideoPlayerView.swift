...
public struct SimpleVideoPlayerView: View {
    ...
    private func progressSlider() -> some View {
        GeometryReader { geo in
            ZStack(alignment: .leading) {
                ...
                bufferedRangesView(width: geo.size.width)
    ...
    @ViewBuilder
    private func bufferedRangesView(width: CGFloat) -> some View {
        ForEach(
            Array(viewModel.bufferedRanges.enumerated()),
            id: \.0
        ) { _, range in

            let startFrac = max(
                0,
                range.lowerBound / max(1.0, viewModel.duration)
            )

            let endFrac = min(
                1,
                range.upperBound / max(1.0, viewModel.duration)
            )

            Rectangle()
                .fill(Color(.systemGray3))
                .frame(
                    width: width * CGFloat(endFrac - startFrac),
                    height: 4
                )
                .offset(
                    x: width * CGFloat(startFrac)
                )
        }
    }
    ...
    private func loadButton() -> some View {
        Button("Load") {
            viewModel.reload(
                autoPlay: viewModel.autoPlayEnabled
            )
        }
        .buttonStyle(.bordered)
    }
